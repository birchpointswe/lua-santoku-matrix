<p align="center">
  <img src="https://santoku.dev/logo-santoku-matrix.png" height="64" alt="santoku-matrix">
</p>

# santoku-matrix

The data layer for santoku: typed numeric containers for machine learning and retrieval
work. Dense matrices (`mtx`), sparse matrices (`csr`), labelled span sets (`spans`), the
zero-indexed typed vectors they are built on, plus mmap-backed vectors and C-level hash
maps and sets.

## Install

```sh
luarocks install santoku-matrix
```
...
local hits = X:rows(ivec.create({ 0 }))

print(hits:nnz())
```

Storage is a flat C buffer: `create` adopts the vectors you hand it rather than copying,
and every operation runs in C over the whole buffer.

## Documentation

Runnable examples and the full API: [santoku.dev](https://santoku.dev/#santoku-matrix).
...
[`test/spec/santoku/matrix.lua`](test/spec/santoku/matrix.lua) (the typed vectors),
[`test/spec/santoku/csr.lua`](test/spec/santoku/csr.lua),
[`test/spec/santoku/mtx.lua`](test/spec/santoku/mtx.lua),
[`test/spec/santoku/spans.lua`](test/spec/santoku/spans.lua), and
[`test/spec/santoku/mmap.lua`](test/spec/santoku/mmap.lua).
