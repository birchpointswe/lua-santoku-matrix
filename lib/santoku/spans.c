...
static int tk_spans_contained_labels_lua (lua_State *L)
{
  ...
  tk_spans_t *A = tk_spans_peek(L, 1, "spans");
  tk_spans_t *G = tk_spans_peek(L, 2, "gold");
  int64_t as = tk_spans_colidx(A, "s"), ae = tk_spans_colidx(A, "e");
  int64_t gs = tk_spans_colidx(G, "s"), ge = tk_spans_colidx(G, "e");
  if (as < 0 || ae < 0 || gs < 0 || ge < 0)
    return tk_lua_verror(L, 2, "spans", "contained_labels requires columns \"s\", \"e\"");
  uint64_t nd = tk_spans_docs(A);
  uint64_t na = tk_spans_n(A);
  tk_ivec_t *out = tk_ivec_create(L, na);
  for (uint64_t i = 0; i < na; i ++)
  ...
  for (uint64_t d = 0; d < nd; d ++) {
    for (int64_t j = A->offsets->a[d]; j < A->offsets->a[d + 1]; j ++) {
      int64_t sj = A->cols[as]->a[j], ej = A->cols[ae]->a[j];
      for (int64_t g = G->offsets->a[d]; g < G->offsets->a[d + 1]; g ++)
        if (sj >= G->cols[gs]->a[g] && ej <= G->cols[ge]->a[g]) {
          out->a[j] = 1; break;
        }
    }
  }
...
static luaL_Reg tk_spans_mt_fns[] = {
  ...
  { "contained_labels", tk_spans_contained_labels_lua },
